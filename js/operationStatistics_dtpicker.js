...
function(e, t) {
	...
	var i = '<div class="mui-poppicker">		<div class="mui-poppicker-header">			<button class="mui-btn mui-poppicker-btn-cancel">取消</button>			<button class="mui-btn mui-btn-blue mui-poppicker-btn-ok" id = "btn-confirm">确定</button>			<div class="mui-poppicker-clear"></div>		</div>		<div class="mui-poppicker-body">		</div>	</div>',
	...
	e.DtPicker = e.Class.extend({
		init: function(n) {
			...
			s.cancel.addEventListener("tap", function() {
				...
				//时间传输
				var dtpickerTime = document.getElementById("result").innerText;	
					localStorage.setItem("operationStatistics_time",dtpickerTime);
					mui.fire(plus.webview.currentWebview().opener(),"dtpicker:close");
					mui.fire(plus.webview.currentWebview().opener(),"operationStatistics:time");
